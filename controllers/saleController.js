...
export default {
  ...
  query: async (req, res, next) => {
    try {
      let reg = await models.Sale.findOne({ _id: req.query._id });
      ...
      if (!reg) {
        ...
        reg = (await models.Sale.findOne({ _id: req.query._id }))
  ...
  list: async (req, res, next) => {
    try {
      ...
      const reg = await models.Sale.find({
        $or: [
          { proofNumber: new RegExp(keyword, "i") },
          { proofSeries: new RegExp(keyword, "i") },
        ],
      })
        .populate("user", { name: 1 })
        .populate("person", {
